...
## 5) Futur cofinal fixé (non imposé par les définitions actuelles)

**Fait formel** : `AutoRegulatedCofinal` et `ObstructionCofinal` (tels qu’ils sont définis dans
`RevHalt/Theory/PrimitiveHolonomy.lean`) commencent par `∃ C : Set P`. Donc la théorie **n’impose pas**
un futur cofinal unique : le témoin `C` fait partie de la donnée d’une preuve.

Conséquence immédiate : toute lecture “sur le même futur” n’est **pas** un théorème de ces définitions ;
pour parler d’un *tail* fixé, il faut changer le type de l’énoncé (au niveau des définitions), par exemple :

- soit en rendant le futur cofinal **paramètre** explicite,
- soit en encapsulant ce futur dans une structure/constante du modèle (un “future object”).

Dans ce cas (futur fixé), on compare sur le même `C` :

- `AutoRegulated sem obs target_obs (CellsOver C)`
- `Obstruction sem obs target_obs (CellsOver C)`

Deux manières standard de formaliser “futur cofinal fixé” (sans axiomes classiques) :

### 5.1 Fixer une timeline interne (un `Scheduling`)

On remplace “`∃ C`” par une chaîne cofinale (un shadow fixé) et on travaille sur son image `C := range sched.c`.
Alors les deux énoncés portent sur **le même** `C`.

### 5.2 Quantifier “pour tout cofinal”

Définir des variantes fortes :

```
AutoRegulatedForallCofinal :=
  ∀ C, Cofinal(C) → AutoRegulated(CellsOver C)

ObstructionForallCofinal :=
  ∀ C, Cofinal(C) → Obstruction(CellsOver C)
```

Dans ces variantes, tu forces des énoncés vraiment globaux (et tu peux alors obtenir des implications
“à la bonne direction” entre cofinalités).

---

## 6) Résumé (forme “théorèmes”)
